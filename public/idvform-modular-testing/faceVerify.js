...
async function fetchUserInfo(token, userIndex) {
  const payload = { token, userIndex };
  const result = await proxyToBackend('face-verify-info', payload, 'GET');
  return result;
}
...
async function uploadAllImages(token, userIndex, images) {
  const payload = { token, userIndex, images: {} };
  for (const [key, blob] of Object.entries(images)) {
    payload.images[key] = await blobToBase64(blob);
  }
  const result = await proxyToBackend('face-verify-upload', payload, 'POST');
  return result;
}
